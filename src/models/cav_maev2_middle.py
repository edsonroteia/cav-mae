...
#         )
    ...
    def forward_decoder(self, x, mask_a, ids_restore_a, mask_v, ids_restore_v):
        ...
        return x_a, x_v[:, (self.num_frames // 2):(self.num_frames // 2 + 1), :]
    ...
    def forward_mae_loss(self, input, pred, mask, modality):
        if modality == 'a':
            ...
            target = self.patchify(input[:, :, input.shape[2]//2, :, :], 3, 1, int(input.shape[3]/self.patch_embed_v.patch_size[0]), int(input.shape[4]/self.patch_embed_v.patch_size[1]), 16)
        ...
        # print("DEBUG: ", pred.shape, target.shape, mask.shape)
        loss = (pred - target) ** 2
