...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cross_x):
        ...
        kv = self.kv(cross_x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class CMP(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.cross_attn1 = CrossAttention(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop)
        self.self_attn = Block(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, drop_path, act_layer, norm_layer)
        self.cross_attn2 = CrossAttention(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop)

    def forward(self, x_a, x_v, modality='a'):
        if modality == None:
            x_a = self.cross_attn1(x_a, x_v)
            x_v = self.cross_attn1(x_v, x_a)
            x_a = self.self_attn(x_a, modality='a')
            x_v = self.self_attn(x_v, modality='v')
            x_a = self.cross_attn2(x_a, x_v)
            x_v = self.cross_attn2(x_v, x_a)
        elif modality == 'a':
            x_a = self.self_attn(x_a, modality='a')
        elif modality == 'v':
            x_v = self.self_attn(x_v, modality='v')
...
class CAVMAEv2(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        self.blocks_v = VisionTransformer(embed_dim=int(embed_dim/2), num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, depth=modality_specific_depth, init_values=0., all_frames=8)
        ...
        self.decoder_proj_v = nn.ConvTranspose3d(
...
#         )
    ...
    def unpatchify(self, x, c, t, h, w, p=16):
        """
    ...
    def forward_encoder(self, a, v, mask_ratio_a, mask_ratio_v, mask_mode='unstructured'):
        ...
        for blk in self.blocks_u:
            ca, cv = blk(a, v, modality='a')
        ca = self.norm_a(ca)

        for blk in self.blocks_u:
            ca, cv = blk(a, v, modality='v')
        cv = self.norm_v(cv)
    ...
    def forward_decoder(self, x, mask_a, ids_restore_a, mask_v, ids_restore_v):
        ...
        x_v = self.decoder_proj_v(x_v)
        # x_v = x_v.reshape(x_v.shape[0], original_x_v_shape[1], -1)
    ...
    def forward(self, audio, imgs, mask_ratio_a=0.75, mask_ratio_v=0.75, mae_loss_weight=1., contrast_loss_weight=0.01, mask_mode='unstructured'):
        ...
        recon_v = torch.einsum('ncthw->nthwc', recon_v)
