...
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None):
        ...
        self.preprocess = CLIPImageProcessor.from_pretrained("openai/clip-vit-base-patch16")
    ...
    def get_image(self, filename, filename2=None, mix_lambda=1):
        if filename2 is None:
            ...
            image_tensor = self.preprocess(img, return_tensors="pt")['pixel_values'][0]
            return image_tensor
        else:
            img1 = Image.open(filename).convert('RGB')
            img2 = Image.open(filename2).convert('RGB')
            
            image_tensor1 = self.preprocess(img1, return_tensors="pt")['pixel_values'][0]
            image_tensor2 = self.preprocess(img2, return_tensors="pt")['pixel_values'][0]
