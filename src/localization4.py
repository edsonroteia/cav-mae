...
class AVLocalization:
    ...
    def generate_relevance(self, model, m_input, index=None):
        ...
        a_input, v_input = m_input
        v_input = v_input.unsqueeze(0).repeat(8, 1, 1, 1, 1).permute(1, 2, 0, 3, 4)
        m_input = (a_input, v_input)
    ...
    def get_map(patches, audio):
        ...
        similarity = similarity.reshape(B, 4, 14, 14)
            ...
            for i, (a_input, v_input, _, bboxes) in enumerate(data_loader):
                ...
                for j in range(len(iou_batch)):
                    ...
                    result = {
                        'iou': iou_batch[j],
                        'auc': auc_batch[j],
                        'image': self.unnormalize(v_input[j][:,0,:,:]).numpy(),
                        'bbox': bboxes[j],
                        'map': lmap,
                        'binary_map': binary_map
                    }
                    if i == 0:
                        self.save_result_image(result, f"{self.model}_imgs/{i+1}_{j}_video.png")
    ...
    def visualize_and_store_results(self, i, v_input, localization_map, bboxes, ious):
        ...
        for j in range(50):
            # import pdb ; pdb.set_trace()
            img = self.unnormalize(v_input[j][:,0,:,:]).numpy()
    ...
    def plot_and_save_images(self, i, j, img, bbox, resized_map, map_binary):
        ...
        img_filename = f"{self.model}_imgs/image_{i}_{j}_video.png"
...
if __name__ == '__main__':
    ...
    for model in tqdm(models_list):
        ...
        # Load model
        audio_model = models.CAVMAEV2(modality_specific_depth=11)
        sdA = torch.load(model, map_location=device)
        if isinstance(audio_model, torch.nn.DataParallel) == False:
            audio_model = torch.nn.DataParallel(audio_model)
        # audio_model.load_state_dict(sdA, strict=False) @TODO: uncomment
        audio_model.eval()
