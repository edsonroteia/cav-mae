...
def get_agg_sim_mat(a, b, strategy='max'):
    ...
    # Split the batch across 8 GPUs
    a_split = torch.chunk(a, 8, dim=0)
    b_split = torch.chunk(b, 8, dim=0)
    ...
    for i in range(8):
        with torch.cuda.device(i):
            ...
            if strategy == 'max' or strategy == 'mean':
                # Compute similarities for all frame pairs using matrix multiplication
                similarities = torch.matmul(a_gpu.view(-1, a_gpu.size(-1)), b_gpu.view(-1, b_gpu.size(-1)).t())
                similarities = similarities.view(a_gpu.size(0), a_gpu.size(1), b_gpu.size(0), b_gpu.size(1))
                
                if strategy == 'max':
                    sim_mat_gpu = similarities.max(dim=(1, 3))[0]
                elif strategy == 'mean':
                    sim_mat_gpu = similarities.mean(dim=(1, 3))
            
            elif strategy == 'diagonal_mean' or strategy == 'diagonal_max':
                # Compare elements from the diagonal of the video submatrix
                diagonal_similarities = torch.einsum('bif,bjf->bij', a_gpu, b_gpu)
                
                if strategy == 'diagonal_mean':
                    sim_mat_gpu = diagonal_similarities.mean(dim=-1)
                elif strategy == 'diagonal_max':
                    sim_mat_gpu = diagonal_similarities.max(dim=-1)[0]
            
            else:
                raise ValueError(f"Unknown aggregation strategy: {strategy}")
            
            sim_mat_split.append(sim_mat_gpu.cpu())
    
    # Combine results from all GPUs
    sim_mat = torch.cat(sim_mat_split, dim=0)
...
if __name__ == "__main__":
    ...
    if dataset == "audioset":
        ...
        for direction in ['video', 'audio']:
            ...
            r1, r5, r10, mr = eval_retrieval(model, data, audio_conf=audio_conf, label_csv=label_csv, num_class=309, direction=direction, model_type=model_type, batch_size=100, strategy=strategy)
            res.append([dataset, direction, r1, r5, r10, mr])
    else:
