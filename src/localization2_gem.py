...
class AVLocalization:
    ...
    def get_map(patches, audio):
        B, _, _ = patches.shape
        similarity = np.einsum('biv,bkv->bik', patches, audio).squeeze(-1)
        # similarity_max = similarity.max(axis=-1, keepdims=True)
        # similarity_min = similarity.min(axis=-1, keepdims=True)
    ...
    def evaluate(self, model, data_loader):
        ...
        with torch.no_grad():
            for i, (a_input, v_input, _, bboxes) in enumerate(tqdm(data_loader)):
                audio_output, video_output = self.forward_pass_patches_audiomean(model, a_input, v_input)
                localization_map = self.get_map(video_output, audio_output)
                ...
                for j in range(len(iou_batch)):
                    ...
                    if i == 0:
                        self.save_result_image(result, f"{self.model}_imgs/{i+1}_{j}_gem_real.png")
    ...
    def forward_pass_patches_audiomean(self, model, a_input, v_input):
        ...
        with autocast():
            _, _, _, _, _, audio_output, video_output = model.module.forward_encoder(audio_input, video_input, 0, 0)
    ...
    def process_map(self, localization_map):
        ...
        resized_map = F.interpolate(heatmap, size=(224, 224), mode='bilinear')
    ...
    def plot_image_with_map_and_bbox(ax, img, lmap, bbox, title):
        ...
        ax.add_patch(rect)
        ax.imshow(lmap, cmap='jet', alpha=0.5)
        ax.set_title(title)
        return ax
    ...
    def plot_and_save_images(self, i, j, img, bbox, resized_map, map_binary):
        ...
        img_filename = f"{self.model}_imgs/image_{i}_{j}_gem.png"
...
if __name__ == '__main__':
    ...
    for model in tqdm(models_list):
        ...
        # Load model
        audio_model = models.CAVMAEGEM(modality_specific_depth=11)
