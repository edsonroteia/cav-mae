...
class AudiosetDataset(Dataset):
    ...
    def pro_data(self, data_json):
        for i in range(len(data_json)):
            data_json[i] = [data_json[i]['wav'], data_json[i]['labels'], data_json[i]['video_id'], data_json[i]['video_path'], str(data_json[i]['bbox']) if 'bbox' in data_json[i] else None]
    ...
    def get_image(self, filenames, mix_lambda=1):
        images = []
        for filename in filenames:
            if filename:
                ...
                images.append(torch.zeros(3, self.im_res, self.im_res))  # Handle missing frames

        # Ensure that the tensor has the expected number of frames
        while len(images) < len(filenames):
            images.append(torch.zeros(3, self.im_res, self.im_res))

        image_tensor = torch.stack(images, dim=0)
        # if mix_lambda != 1 and len(filenames) > 1:  # For mixup case
        #     img2 = Image.open(filenames[1])
        #     image_tensor2 = self.preprocess(img2)
        #     image_tensor = mix_lambda * image_tensor + (1 - mix_lambda) * image_tensor2.unsqueeze(0)
        return image_tensor.permute(1,0,2,3)
    ...
    def randselect_img(self, video_id, video_path, num_video_frames=8):
        frame_indices = []
        if self.mode == 'eval':
            if self.frame_use == -1:
                #Get middle segment
                start_frame = max(0, int(self.total_frame / 2) - num_video_frames // 2)
                frame_indices = [start_frame + i for i in range(num_video_frames)]
            else:
                start_frame = self.frame_use
                frame_indices = [start_frame + i for i in range(num_video_frames)]
        else:
            start_frame = random.randint(0, self.total_frame - num_video_frames)
            frame_indices = [start_frame + i for i in range(num_video_frames)]

        paths = []
        for idx in frame_indices:
            frame_path = video_path + '/frame_' + str(idx) + '/' + video_id + '.jpg'
            if os.path.exists(frame_path):
                paths.append(frame_path)
            else:
                paths.append(None)  # or handle missing frame differently
        return paths
    ...
    def __getitem__(self, index):
        if random.random() < self.mixup:
            ...
            image = self.get_image(image_paths)

            # except:
            #     image = torch.zeros([8, 3, self.im_res, self.im_res]) + 0.01 # Adjust this for the number of frames
            ...
            # plot here 2
            image_paths = self.randselect_img(datum['video_id'], datum['video_path'], num_video_frames=8)
            image = self.get_image(image_paths, 0)
        ...
        return fbank, image, label_indices#, datum['bbox'] if 'bbox' in datum else None
