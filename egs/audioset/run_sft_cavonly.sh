...
#SBATCH --output=./log/%j_sft_cavonly.txt
...
# Pretrained model: Contrastive-only (CAV-only)
pretrain_path=/weka/kuehne/kqr867/code/cav-mae/egs/audioset/exp/contrastive-only-audioset-cav-mae-balNone-lr1e-4-epoch25-bs120-mr-unstructured-0.75/models/best_audio_model.pth
...
exp_dir=./exp/sft-cavonly-${lr}-bs${batch_size}-epoch${epoch}-$(date +%Y%m%d_%H%M%S)
mkdir -p $exp_dir

echo "=========================================="
echo "SFT for CAV-only (Contrastive-only) Model"
