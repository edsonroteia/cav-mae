...
#SBATCH --output=./log/%j_sft_scalepp.txt
...
# Pretrained model: Original CAV-MAE Scale++ (from Yuan Gong et al.)
# Paper settings: batch_size=256, lambda_c=0.01, 75% unstructured masking
pretrain_path=/weka/kuehne/kqr867/code/cav-mae/egs/audioset/cav-mae-scale++.pth

# Fine-tuning hyperparameters (same as our other models for fair comparison)
freeze_base=False
...
exp_dir=./exp/sft-scalepp-original-${lr}-bs${batch_size}-epoch${epoch}-$(date +%Y%m%d_%H%M%S)
mkdir -p $exp_dir

echo "=========================================="
echo "SFT for Original CAV-MAE Scale++ Model"
