...
#SBATCH --output=log/%j_sft_merged_plusplus.txt
#SBATCH --error=log/%j_sft_merged_plusplus.err
...
# Select the merged model to fine-tune (set to best model after retrieval evaluation)
# Options: merged_simple, merged_weighted_X, merged_fisher, merged_task_arith_X, merged_orthogonal, merged_layerwise
MERGED_MODEL_NAME=${1:-"merged_weighted_0.5"}  # Default or pass as argument

# Pretrain path
MERGED_DIR=/weka/kuehne/kqr867/code/cav-mae/egs/audioset/exp/merged-models-plusplus
pretrain_path=$MERGED_DIR/${MERGED_MODEL_NAME}.pth

# Verify model exists
if [[ ! -f "$pretrain_path" ]]; then
    echo "ERROR: Model not found at $pretrain_path"
...
# ========================================
# SFT CONFIGURATION (from run_sft_scalepp.sh)
# ========================================
model=cav-mae-ft
ftmode=multimodal
...
exp_dir=/weka/kuehne/kqr867/code/cav-mae/egs/audioset/exp/sft-${MERGED_MODEL_NAME}-5e-5-bs36-epoch15-$(date +%Y%m%d_%H%M%S)
mkdir -p $exp_dir

echo "=========================================="
echo "SFT for Merged++ Model: $MERGED_MODEL_NAME"
...
echo ""
echo "=========================================="
echo "SFT completed for: $MERGED_MODEL_NAME"
