#!/bin/bash
#SBATCH --job-name="cavmae-as2m-joint"
...
#SBATCH --output=./log/%j_joint.txt
#SBATCH --error=./log/%j_joint.err

# CAV-MAE pretraining with JOINT objective (MAE + Contrastive)
# This is the baseline for comparison with separate objective training
...
contrast_loss_weight=0.01  # Standard joint training ratio
mae_loss_weight=1.0
tr_pos=False
norm_pix_loss=True
...
exp_dir=./exp/joint-${dataset}-${model}-bal${bal}-lr${lr}-epoch${epoch}-bs${batch_size}-norm${norm_pix_loss}-c${contrast_loss_weight}-p${mae_loss_weight}-mr-${mask_mode}-${masking_ratio}
mkdir -p $exp_dir
