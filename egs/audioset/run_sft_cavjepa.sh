...
#SBATCH --output=./log/%j_sft_cavjepa.txt
...
# Pretrained model: CAV-JEPA (JEPA + Contrastive)
pretrain_path=/weka/kuehne/kqr867/code/cav-mae/egs/audioset/exp/cavjepa-audioset-lr1e-4-epoch25-bs120-mr0.75-mom0.996-0.999-pred4/models/best_audio_model.pth
...
exp_dir=./exp/sft-cavjepa-${lr}-bs${batch_size}-epoch${epoch}-$(date +%Y%m%d_%H%M%S)
mkdir -p $exp_dir

echo "=========================================="
echo "SFT for CAV-JEPA Model"
echo "Pretrain path: ${pretrain_path}"
echo "Exp dir: ${exp_dir}"
echo "=========================================="

CUDA_CACHE_DISABLE=1 python -W ignore /weka/kuehne/kqr867/code/cav-mae/src/run_cavjepa_ft.py \
    --model ${model} \
    --dataset ${dataset} \
    --data-train ${tr_data} \
    --data-val ${te_data} \
