...
#SBATCH --output=./log/%j_sft_scalep.txt
...
# Paper settings: batch_size=108, lambda_c=0.01, 75% unstructured masking
...
exp_dir=./exp/sft-scalep-original-${lr}-bs${batch_size}-epoch${epoch}-$(date +%Y%m%d_%H%M%S)
mkdir -p $exp_dir
