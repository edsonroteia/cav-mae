...
#SBATCH --output=./log/%j_mae++.txt
#SBATCH --error=./log/%j_mae++.err

# MAE++ : Stronger MAE-only baseline
# - Batch size: 256 (vs 120 in original)
# - Learning rate: 2e-4 (keep - critical for LayerNorm stability)
...
bal=None
lr=2e-4          # Keep 2e-4 (critical for LayerNorm stability)
epoch=25
...
exp_dir=./exp/mae++-${dataset}-${model}-bal${bal}-lr${lr}-epoch${epoch}-bs${batch_size}-mr-${mask_mode}-${masking_ratio}
mkdir -p $exp_dir

echo "Starting MAE++ training"
